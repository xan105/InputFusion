...
STDMETHODIMP IDirectInput8A::QueryInterface(REFIID riid, void** ppvObject) {
  ...
  if (IsEqualGUID(riid, IID_IDirectInputJoyConfig8)) {
    SDL_Log("IDirectInput8A::QueryInterface() > IDirectInputJoyConfig");
  ...
  SDL_Log("IDirectInput8A::QueryInterface() > Unknow REFIID: %s", GUIDToString(riid).c_str());
...
STDMETHODIMP IDirectInput8A::EnumDevices(DWORD dwDevType, LPDIENUMDEVICESCALLBACKA lpCallback, LPVOID pvRef, DWORD dwFlags) {
  ...
  //These are placeholder gamepads so we can connect/disconnect while playing
  //Problem: A lot of games completely crash when they are more than one controller 😩 (FIXME)
  for (int i = 0; i < DIRECTINPUT_USER_MAX_COUNT; i++) { 
      ...
      SDL_Log("IDirectInput8A::EnumDevices() > Created %s", XBOX360_INSTANCE_NAMEA);

      if (lpCallback(&lpddi, pvRef) == DIENUM_STOP) { break; }
  }
  
  /*
  TO-DO: Also need to figure out KBM. 
  NB: Always report controller first. DInput make no assumptions on the order of the devices but some games do and expect the controller to be first.
  */
