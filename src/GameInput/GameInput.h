...
DEFINE_GUID(IID_IGameInput, 0x11BE2A7E, 0x4254, 0x445A, 0x9C, 0x09, 0xFF, 0xC4, 0x0F, 0x00, 0x69, 0x18);
DEFINE_GUID(IID_IGameInputDevice, 0x31DD86FB, 0x4C1B, 0x408A, 0x86, 0x8F, 0x43, 0x9B, 0x3C, 0xD4, 0x71, 0x25);
DEFINE_GUID(IID_IGameInputDispatcher, 0x415EED2E, 0x98CB, 0x42C2, 0x8F, 0x28, 0xB9, 0x46, 0x01, 0x07, 0x4E, 0x31);
DEFINE_GUID(IID_IGameInputForceFeedbackEffect, 0x51BDA05E, 0xF742, 0x45D9, 0xB0, 0x85, 0x94, 0x44, 0xAE, 0x48, 0x38, 0x1D);
DEFINE_GUID(IID_IGameInputReading, 0x2156947A, 0xE1FA, 0x4DE0, 0xA3, 0x0B, 0xD8, 0x12, 0x93, 0x1D, 0xBD, 0x8D);

typedef uint64_t GameInputCallbackToken;

// Enum

typedef enum GameInputMousePositions
{
...
typedef void (CALLBACK* GameInputReading_Callback)(_In_ GameInputCallbackToken callbackToken, _In_ void* context, _In_ IGameInputReading* reading, _In_ bool hasOverrunOccurred);
typedef void (CALLBACK* GameInputDevice_Callback)(_In_ GameInputCallbackToken callbackToken, _In_ void* context, _In_ IGameInputDevice* device, _In_ uint64_t timestamp, _In_ GameInputDeviceStatus currentStatus, _In_ GameInputDeviceStatus previousStatus);
typedef void (CALLBACK* GameInputGuideButton_Callback)(_In_ GameInputCallbackToken callbackToken, _In_ void* context, _In_ IGameInputDevice* device, _In_ uint64_t timestamp, _In_ bool isPressed);
typedef void (CALLBACK* GameInputKeyboardLayout_Callback)(_In_ GameInputCallbackToken callbackToken, _In_ void* context, _In_ IGameInputDevice* device, _In_ uint64_t timestamp, _In_ uint32_t currentLayout, _In_ uint32_t previousLayout);
typedef void (CALLBACK* GameInputSystemButton_Callback)(_In_ GameInputCallbackToken callbackToken, _In_ void* context, _In_ IGameInputDevice* device, _In_ uint64_t timestamp, _In_ GameInputSystemButtons currentButtons, _In_ GameInputSystemButtons previousButtons);
...
public:
    ...
    virtual STDMETHODIMP_(bool) Dispatch(uint64_t quotaInMicroseconds);
    virtual STDMETHODIMP OpenWaitHandle(HANDLE* waitHandle);
};
...
public:
    ...
    virtual STDMETHODIMP_(ULONG) AddRef() override;
    virtual STDMETHODIMP_(ULONG) Release() override;
    virtual STDMETHODIMP_(bool) GetArcadeStickState(GameInputArcadeStickState* state);
    virtual STDMETHODIMP_(uint32_t) GetControllerAxisCount();
    virtual STDMETHODIMP_(uint32_t) GetControllerAxisState(uint32_t stateArrayCount, float* stateArray);
    virtual STDMETHODIMP_(uint32_t) GetControllerButtonCount();
    virtual STDMETHODIMP_(uint32_t) GetControllerButtonState(uint32_t stateArrayCount, bool* stateArray);
    virtual STDMETHODIMP_(uint32_t) GetControllerSwitchCount();
    virtual STDMETHODIMP_(uint32_t) GetControllerSwitchState(uint32_t stateArrayCount, GameInputSwitchPosition* stateArray);
    virtual STDMETHODIMP_(void) GetDevice(IGameInputDevice** device);
    virtual STDMETHODIMP_(bool) GetFlightStickState(GameInputFlightStickState* state);
    virtual STDMETHODIMP_(bool) GetGamepadState(GameInputGamepadState* state);
    virtual STDMETHODIMP_(GameInputKind) GetInputKind();
    virtual STDMETHODIMP_(uint32_t) GetKeyCount();
    virtual STDMETHODIMP_(uint32_t) GetKeyState(uint32_t stateArrayCount, GameInputKeyState* stateArray);
    virtual STDMETHODIMP_(bool) GetMouseState(GameInputMouseState* state);
    virtual STDMETHODIMP_(bool) GetRacingWheelState(GameInputRacingWheelState* state);
    virtual STDMETHODIMP_(uint64_t) GetTimestamp();
    virtual STDMETHODIMP_(bool) GetUiNavigationState(GameInputUiNavigationState* state);
};
...
public:
    ...
    virtual STDMETHODIMP_(ULONG) AddRef() override;
    virtual STDMETHODIMP_(ULONG) Release() override;
    virtual STDMETHODIMP CreateForceFeedbackEffect(uint32_t motorIndex, const GameInputForceFeedbackParams* params, IGameInputForceFeedbackEffect** effect);
    virtual STDMETHODIMP GetDeviceInfo(const GameInputDeviceInfo** info);
    virtual STDMETHODIMP_(GameInputDeviceStatus) GetDeviceStatus();
    virtual STDMETHODIMP_(bool) IsForceFeedbackMotorPoweredOn(uint32_t motorIndex);
    virtual STDMETHODIMP_(void) SetForceFeedbackMotorGain(uint32_t motorIndex, float masterGain);
    virtual STDMETHODIMP_(void) SetRumbleState(const GameInputRumbleParams* params);
};
...
public:
    ...
    virtual STDMETHODIMP_(ULONG) Release() override;
    virtual STDMETHODIMP_(void) GetDevice(IGameInputDevice** device);
    virtual STDMETHODIMP_(float) GetGain();
    virtual STDMETHODIMP_(uint32_t) GetMotorIndex();
    virtual STDMETHODIMP_(void) GetParams(GameInputForceFeedbackParams* params);
    virtual STDMETHODIMP_(GameInputFeedbackEffectState) GetState();
    virtual STDMETHODIMP_(void) SetGain(float gain);
    virtual STDMETHODIMP_(bool) SetParams(const GameInputForceFeedbackParams* params);
    virtual STDMETHODIMP_(void) SetState(GameInputFeedbackEffectState state);
};
...
public:
    ...
    virtual STDMETHODIMP_(ULONG) AddRef() override;
    virtual STDMETHODIMP_(ULONG) Release() override;
    virtual STDMETHODIMP CreateDispatcher(IGameInputDispatcher** dispatcher);
    virtual STDMETHODIMP FindDeviceFromId(const APP_LOCAL_DEVICE_ID* value, IGameInputDevice** device);
    virtual STDMETHODIMP FindDeviceFromPlatformString(LPCWSTR value, IGameInputDevice** device);
    virtual STDMETHODIMP GetCurrentReading(GameInputKind inputKind, IGameInputDevice* device, IGameInputReading** reading);
    virtual STDMETHODIMP_(uint64_t) GetCurrentTimestamp();
    virtual STDMETHODIMP GetNextReading(IGameInputReading* referenceReading, GameInputKind inputKind, IGameInputDevice* device, IGameInputReading** reading);
    virtual STDMETHODIMP GetPreviousReading(IGameInputReading* referenceReading, GameInputKind inputKind, IGameInputDevice* device, IGameInputReading** reading);
    virtual STDMETHODIMP RegisterDeviceCallback(IGameInputDevice* device, GameInputKind inputKind, GameInputDeviceStatus statusFilter, GameInputEnumerationKind enumerationKind, void* context, GameInputDevice_Callback callbackFunc, GameInputCallbackToken* callbackToken);
    virtual STDMETHODIMP RegisterKeyboardLayoutCallback(IGameInputDevice* device, void* context, GameInputKeyboardLayout_Callback callbackFunc, GameInputCallbackToken* callbackToken);
    virtual STDMETHODIMP RegisterReadingCallback(IGameInputDevice* device, GameInputKind inputKind, void* context, GameInputReading_Callback callbackFunc, GameInputCallbackToken* callbackToken);
    virtual STDMETHODIMP RegisterSystemButtonCallback(IGameInputDevice* device, GameInputSystemButtons buttonFilter, void* context, GameInputSystemButton_Callback callbackFunc, GameInputCallbackToken* callbackToken);
    virtual STDMETHODIMP_(void) SetFocusPolicy(GameInputFocusPolicy policy);
    virtual STDMETHODIMP_(void) StopCallback(GameInputCallbackToken callbackToken);
